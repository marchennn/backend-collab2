...
class MQTT:
    def __init__(self, broker_address: str):
        self.broker_address = "192.168.0.106"
    ...
    async def process_message(self, topic: str, payload: str):
        try:
            ...
            try:
                ...
                # Jika payload bukan JSON, anggap sebagai string biasa
                data = {"value": payload}
            ...
            elif topic in ["led/1", "led/2", "led/3", "led/4", "led/5"]:
                led_key = topic.split("/")[1]
                led_value = data.get(f"led{led_key}", data.get("value", "off"))  # Default ke "off" jika tidak ada
                pwm_led_value = data.get(f"pwmLed{led_key}", "0")  # Default ke "0" jika tidak ada
    ...
    async def subscribe(self):
        topics = ["sensor/data", "led/1", "led/2", "led/3", "led/4", "led/5"]
...
async def main():
    ...
    mqtt_client = MQTT(broker_address="192.168.0.106")
